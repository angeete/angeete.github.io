...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
     ...
     if(empty_impl(node)){
       ...
       height = 1+max(height_impl(node->left), height_impl(node->right));
     }
     return height;
  }
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    if(empty_impl(node)){
      ...
      a->datum = node->datum;
      a->left = copy_nodes_impl(node->left);
      a->right = nullptr;
    } else if (!(node->left)) {
      a->datum = node->datum;
      a->right = copy_nodes_impl(node->right);
      a->left = nullptr;
    } else {
      a->datum = node->datum;
      a->left = copy_nodes_impl(node->left);
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    if(empty_impl(node)){
      return true;
    } else if (size_impl(node)==1){
      return true;
    }
    bool left = true;
    bool right = true;
    if(node->left && node->right){
      left = less(node->left->datum, node->datum) && 
      check_sorting_invariant_impl(node->left, less);
      right = less(node->datum, node->right->datum) && 
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    } else if(!less(val,node->datum) && !less(val,node->datum)){
      return min_greater_than_impl(node->right,val,less);
    } else {
