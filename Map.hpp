...
public:
  ...
  Iterator find(const Key_type& k) const{
    return bst.find(std::make_pair(k, Value_type()));
  }
  ...
  Value_type& operator[](const Key_type& k){
    Iterator a;
     if(bst.find(std::make_pair(k, Value_type())) == bst.end()){
        a =  bst.insert(std::make_pair(k, Value_type()));
        return a->second;
     } else {
        a = bst.find(std::make_pair(k, Value_type()));
        return a->second;
     }
